...
export default function EditorPage() {
      ...
      if (playMode === "both") {
        ...
        for (const idx of [0, 1]) {
          if (engineRefs[idx].current) {
            ...
            a.href = url;
            a.download = `${project?.name || "export"}-lane${idx + 1}.wav`;
            a.click();
            URL.revokeObjectURL(url);
          }
        }
      } else {
        const idx = playMode as number;
        if (engineRefs[idx].current) {
          const blob = await engineRefs[idx].current!.exportWAV();
          const url = URL.createObjectURL(blob);
          const a = document.createElement("a");
  ...
  const renderLane = (laneIdx: number) => {
    ...
    return (
      <div className={`flex-1 min-h-0 flex flex-col ${laneIdx === 0 ? "border-b border-border" : ""}`}>
        {/* Lane header */}
